...
const cityLinkKeyMap: Record<string, string> = {
  bangkok: 'bangkok',
  phuket: 'phuket',
  'chiang mai': 'chiangMai',
};

export default function KlookWidget({ city, className = '' }: KlookWidgetProps) {
  const klook = getAffiliate('klook');

  if (!klook) return null;

  const cityKey = city.toLowerCase();
  const activities = cityActivitiesData[cityKey] || cityActivitiesData.bangkok;
  const links = klook.links || {};
  const linkKey = cityLinkKeyMap[cityKey] || 'thailand';
  const cityLink = links[linkKey] || links.thailand || klook.url;

  return (
    <div className={`bg-gradient-to-br from-orange-50 to-red-50 dark:from-gray-800 dark:to-gray-800 rounded-lg p-6 ${className}`}>
