...
/**
 * Generate sitemap index with proper chunking and validation
 */
async function generateSitemapIndex(): Promise<void> {
  const today = new Date().toISOString();
  
  // Get actual lastmod dates from the files
  const getLastMod = async (filename: string): Promise<string> => {
    try {
      const stats = await fs.promises.stat(path.join(PUBLIC_DIR, filename));
  ...
  const sitemaps = [
    { file: 'main-sitemap.xml', changefreq: 'daily' },
    { file: 'blog-sitemap.xml', changefreq: 'daily' },
    { file: 'locations-sitemap.xml', changefreq: 'weekly' },
    { file: 'service-areas-sitemap.xml', changefreq: 'weekly' },
  ];

  const sitemapPromises = sitemaps.map(async (sitemap) => {
    const lastmod = await getLastMod(sitemap.file);
    return `  <sitemap>
    <loc>${BASE_URL}/${sitemap.file}</loc>
    <lastmod>${lastmod}</lastmod>
    <!-- Crawl this sitemap ${sitemap.changefreq} -->
  </sitemap>`;
  });

  const sitemapEntries = await Promise.all(sitemapPromises);
...
${sitemapEntries.join('\n')}
  ...
  // Create a simple XSL stylesheet for better sitemap visualization
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    <html>
      ...
      <body>
        <h1>GoFlyzo XML Sitemap Index</h1>
        <table>
          <tr>
            <th>URL</th>
            <th>Last Modified</th>
  ...
  fs.writeFileSync(path.join(PUBLIC_DIR, 'sitemap.xsl'), xslContent);
}
